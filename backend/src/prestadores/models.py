...
class PrestadorVeterinario(models.Model):
    prestador = models.ForeignKey(Prestador, on_delete=models.CASCADE)
    veterinario = models.ForeignKey(Veterinario, on_delete=models.CASCADE)
    data_vinculo = models.DateField(auto_now_add=True, blank=True, null=True)

    def __str__(self) -> str:
        return f"{self.prestador.user.first_name} {self.veterinario.user.username}"
